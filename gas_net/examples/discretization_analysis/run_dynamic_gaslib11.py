# -*- coding: utf-8 -*-
"""
Created on Mon Apr 29 15:40:15 2024
...
def run_model(num_elements_x):
    network_data_path = r'C:\\Users\\ssnaik\\Biegler\\gas_networks_italy\\gas_networks\\gas_net\\data\\data_files\\Gaslib_11\\networkData.xlsx'
    input_data_path = r'C:\\Users\\ssnaik\\Biegler\\gas_networks_italy\\gas_networks\\gas_net\\data\\data_files\\Gaslib_11\\inputData.xlsx'
    ...
    m_steady.wSource["entry01", :].fix()
    m_steady.wSource["entry02", :].fix()
    m_steady.pSource["entry03", :].fix()
    ...
    # fix pressure source
    m_dyn.wSource["entry01", :].fix()
    m_dyn.wSource["entry02", :].fix()
    m_dyn.pSource["entry03", :].fix()
...
if __name__ == "__main__":
    objective_dynamic = []
    pressure_node_exit03 = {}
    dx_list =[2, 3, 4, 5, 10, 20, 30]
    for num_elements in dx_list:
        ...
        pressure_node_exit03[num_elements] = []
        for t in m_dyn.Times:
            pressure_node_exit03[num_elements].append(pyo.value(m_dyn.node_p['exit03', t]))
    ...
    plt.figure()
    for key in pressure_node_exit03.keys():
        plt.plot(pressure_node_exit03[key], label = key)
    plt.xlabel("Number of finite elements in pipes")
