...
def PIPE_mass_constr(
        m, scale, dt, method = 'BACKWARD', dynamic=True, infinite_horizon=False):
    ...
    ############## inf horizon mass balance #########################  
    def PIPE_mass_balance_infhorizon_rule(
            m, p, vol, t): # 1-->N-1 
        if t == m.Times.first():
            return pyo.Constraint.Skip
        else:
            ...
            ############## constraint #########################              
            return m.gamma*(1 - t**2)*m.drhodt[p, vol+1, t]*V/(N-1)/3600 == (w*scale["w"] - w_next*scale["w"] - cons*scale["w"])
