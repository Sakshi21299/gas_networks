...
file_name = r"final_paper_kai_enmpc_explicit_terminal_each_point_stability_72hrs.xlsx"
file_path = os.path.join(path, file_name)
df_nominal = pd.read_excel(file_path, sheet_name=None, index_col="Unnamed: 0")
...
ax.plot(df_nominal['wCons'][df_nominal['wCons'].columns[1]], 'blue', label = '$\mathrm{Nominal \: demand}$')
...
ax.set_ylim(25,49)
fig.tight_layout()
fig.savefig(os.path.join(savefig_path, "kai-target-demand-enmpc-both.pdf"))
